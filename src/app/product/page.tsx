    ...
    return (
        <form className="form-search shadow-md mb-3 text-gray-600" onSubmit={onsubmit}>
            <label htmlFor="search">
                <input required autoComplete="off" placeholder="search your chats" id="search" type="text" onChange={(e: ChangeEvent<HTMLInputElement>) => setSearch(e.target.value)} value={search} />
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            const queryParams = new URLSearchParams({
                ...
                skip: skip.toString(),
                select: 'title,price,stock,rating',
                ...(q && { q }),
            });

            const url = `${baseUrl}/products/${q ? '/search' : ''}${queryParams.toString() ? `?${queryParams.toString()}` : ''}`;
            ...
            <div className="flex w-full gap-3 mb-3 flex-wrap">
                <div className=' flex-auto shadow-md p-3 rounded-xl bg-white transition-all relative'>
                    <table className="min-w-full">
                        ...
                        <tbody>
                            {products.map(({ title, price, stock, rating }: Product, i: number) => (
                                <tr key={i} className='hover:bg-gray-100'>
                                    ...
                                    <td className="py-2 px-4 text-center"><button className=" rounded-lg border border-gray-800 p-1 hover:bg-gray-800 hover:text-white">Detail</button></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
